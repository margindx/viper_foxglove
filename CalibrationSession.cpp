...
StepInfo describeStep(CalibrationStep step) {
    switch (step) {
        ...
        case CalibrationStep::Straightedge:
            return {step, "3 of 3: straightedge",
                    "Keep the face flat on the surface and butt its long (10 mm) edge against a "
                    "straightedge. Slide along the straightedge and reseat a few times, keeping "
                    "the same edge in contact throughout."};
...
const std::vector<CalibrationSample> &CalibrationSession::samplesFor(CalibrationStep step) const {
    switch (step) {
        ...
        case CalibrationStep::Straightedge: return edgeSamples_;
...
bool CalibrationSession::advance() {
    ...
    switch (step_) {
        ...
        case CalibrationStep::FlatPlacements: step_ = CalibrationStep::Straightedge; break;
        case CalibrationStep::Straightedge: step_ = CalibrationStep::Done; break;
        case CalibrationStep::Done: break;
    }
...
std::optional<CalibrationOutcome> CalibrationSession::solve() const {
    ...
    const auto longAxis = solveCommonDirection(edgeSamples_);
    if (!longAxis.has_value())
        return std::nullopt;

    CalibrationOutcome outcome;
    outcome.pivot = *pivot;
    outcome.faceNormal = *faceNormal;
    outcome.longAxis = *longAxis;
    ...
    outcome.tipRotation = tipRotationFromAxes(face, longAxis->sensorDirection);
